...
class AccesoDatos {
   ...
   // Constructor privado  Patron singleton
    ...
    public function getClientes ($primero,$cuantos,$orden):array {
        $tuser = [];
    ...
    public function getClienteSiguiente($ordenacion, $dato){
        ...
        $cli = false;
        $stmt_cli   = $this->dbh->prepare("select * from Clientes where $ordenacion >'$dato' order by $ordenacion limit 1");
        $stmt_cli->setFetchMode(PDO::FETCH_CLASS, 'Cliente');
        if ( $stmt_cli->execute() ){
            if ( $obj = $stmt_cli->fetch()){
               $cli= $obj;
    ...
    public function getClienteAnterior($ordenacion, $dato){
        ...
        $cli = false;
        $stmt_cli   = $this->dbh->prepare("select * from Clientes where $ordenacion <'$dato' order by $ordenacion DESC limit 1");
        $stmt_cli->setFetchMode(PDO::FETCH_CLASS, 'Cliente');
        if ( $stmt_cli->execute() ){
           if ( $obj = $stmt_cli->fetch()){
              $cli= $obj;
    ...
    //COMPROBAR si la contrasela y login son correctas
    public function comprobarContra_Login($login,$contra):bool
    {
        $correcto = false;

        $stmt_pswd = $this->dbh->prepare("SELECT * FROM InfoClientes WHERE login = '$login' AND contraseña_cif = '$contra'");
        $stmt_pswd->execute();
        $correcto = ($stmt_pswd->rowCount () == 1);
    ...
    public function recogerRol($login)
    {
        $rol = false;

        $stmt_rol = $this->dbh->prepare("SELECT rol FROM InfoClientes WHERE login = '$login'");
        $stmt_rol->execute();
        if ($stmt_rol->rowCount() == 1) {
            // Si se encontró exactamente una fila, obtenemos el valor de 'rol'
