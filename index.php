...
//Verificar si ya entro en la sesión
...
else
{
    ...
    {
        if (verificarCredenciales($_POST['login'], $_POST['password']) && conseguirRol($_POST['login'])) 
            {
                $_SESSION['login'] = true;
            ...
            {
                if($_SESSION['intentos'] < 2) //Si se esupera los 3 intentos se  bloquea el acceso
                {
                    $_SESSION['intentos']++;
                }
                else
                {
                    echo  "<script>alert('Se han superado los intentos permitidos, reinicie el navegador');</script>";
                }
            }
